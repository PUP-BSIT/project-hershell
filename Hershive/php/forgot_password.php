...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user_email === '') {
        ...
        handlePasswordReset($user_email, $conn);
    }
}

function handlePasswordReset($user_email, $conn) {
    global $step, $message, $show_resend, $remaining_time, $show_modal;
    ...
    if (checkRecentEmail($user_id, $conn)) {
        return;
    }

    sendPasswordResetEmail($user_id, $first_name, $user_email, $conn);
}

function checkRateLimit($user_id, $conn) {
    global $remaining_time, $show_modal, $step;
...
function checkRecentEmail($user_id, $conn) {
    global $step, $message, $show_resend;
...
function sendPasswordResetEmail($user_id, $first_name, $user_email, $conn) {
    global $step, $message;

    invalidatePreviousTokens($user_id, $conn);
    ...
    $insert->bind_param('isss', $user_id, $token_hash, $expires_at, $created_at);
    ...
    $email_sent = sendResetEmail($user_email, $first_name, $raw_token);
    ...
    header('Location: email_sent.php?email=' . urlencode($user_email));
    exit;
}

function invalidatePreviousTokens($user_id, $conn) {
...
function sendResetEmail($user_email, $first_name, $raw_token) {
    $base_url = 'https://hershive.com/project-hershell/Hershive/php/';
    $reset_link = $base_url . 'create_new_password.php?token=' . urlencode($raw_token);

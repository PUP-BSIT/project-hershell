...
WHERE p.post_id = ? AND p.deleted = 0 AND sharer.deleted_account = 0
LIMIT 1
";
...
if ($row = $result->fetch_assoc()) {
    ...
    if ($visibility === 'public' || $current_user_id == $postUserId) {
        $shouldShow = true;
    ...
    if (!$shouldShow) {
        echo json_encode(["success" => false, "error" => "You do not have permission to view this post"]);
        exit;
    }

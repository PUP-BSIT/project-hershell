...
$sql = "
    SELECT
        ...
        p.created_at,
        p.visibility,
        (SELECT COUNT(*) FROM heart_react hr WHERE hr.post_id = p.post_id) as likes_count,
    ...
    LEFT JOIN heart_react hr_user ON p.post_id = hr_user.post_id AND hr_user.user_id = ?
...
while ($row = $result->fetch_assoc()) {
    ...
    // Determine media type from URL if exists
    if ($row['media_url']) {
        $extension = strtolower(pathinfo($row['media_url'], PATHINFO_EXTENSION));
        $row['media_type'] = in_array($extension, ['mp4', 'mov', 'avi', 'webm']) ? 'video' : 'image';
    } else {
        $row['media_type'] = null;
    }
...
$count_stmt = $conn->prepare("SELECT COUNT(*) as total FROM post WHERE visibility = 'public'");
$count_stmt->execute();
